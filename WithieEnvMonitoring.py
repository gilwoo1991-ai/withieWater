    ...
    (function() {
        try {
            ...
            function setViewport() {
                let vp = pd.querySelector('meta[name="viewport"]');
                if (!vp) {
                    vp = pd.createElement('meta');
                    vp.name = 'viewport';
                    pd.head.appendChild(vp);
                }
                vp.setAttribute('content', 'width=650, initial-scale=1.0, minimum-scale=0.3, maximum-scale=5.0, user-scalable=yes');
            }
            setViewport();
            // Streamlit이 로드 후 viewport를 덮어쓸 수 있으므로 여러 시점에 재적용
            [50, 200, 500, 1200, 2500].forEach(t => setTimeout(setViewport, t));

            // ── 2. parent head에 고명시도 CSS 직접 주입 ─────────────────
            if (!pd.getElementById('withie-mobile-fix')) {
                const s = pd.createElement('style');
                s.id = 'withie-mobile-fix';
                s.textContent = [
                    'html { min-width: 650px !important; overflow-x: auto !important; }',
                    'body { overflow-x: auto !important; -webkit-overflow-scrolling: touch !important; }',
            ...
            // ── 3. 컬럼 너비 인라인 고정 ────────────────────────────────
            function fixLayout() {
                pd.querySelectorAll('div[data-testid="stHorizontalBlock"]').forEach(b => {
                    b.style.setProperty('flex-direction', 'row', 'important');
                    b.style.setProperty('flex-wrap', 'nowrap', 'important');
                });
                pd.querySelectorAll('div[data-testid="column"]').forEach(col => {
                    const w = col.style.width;
                    // 100%가 아닌 정상 값이 처음 감지되면 저장
                    if (w && w !== '100%' && !col.dataset.ow) col.dataset.ow = w;
                    if (col.dataset.ow) {
                        col.style.setProperty('width', col.dataset.ow, 'important');
                        col.style.setProperty('flex', '0 0 ' + col.dataset.ow, 'important');
                    }
                });
            }
            fixLayout();
            [100, 400, 900, 2000].forEach(t => setTimeout(fixLayout, t));
            new MutationObserver(fixLayout).observe(pd.body, {
                childList: true, subtree: true,
                attributes: true, attributeFilter: ['style', 'class']
            });

            // ── 4. 모바일 전체화면 버튼 ─────────────────────────────────
            if (!pd.getElementById('mobile-fs-btn')) {
                const btn = pd.createElement('div');
                btn.id = 'mobile-fs-btn';
                btn.innerHTML = '⛶';
                btn.title = '전체화면 전환';
                Object.assign(btn.style, {
                    display: window.parent.innerWidth <= 900 ? 'block' : 'none',
                    position: 'fixed', bottom: '20px', right: '20px',
                    width: '50px', height: '50px',
                    backgroundColor: 'rgba(0,0,0,0.7)', color: '#00d4ff',
                    border: '2px solid #00d4ff', borderRadius: '50%',
                    zIndex: '9999999', fontSize: '24px', lineHeight: '46px',
                    textAlign: 'center', cursor: 'pointer',
                    boxShadow: '0 0 10px #00d4ff', backdropFilter: 'blur(4px)',
                    transition: 'all 0.3s ease-in-out'
                });
                window.parent.addEventListener('resize', () => {
                    btn.style.display = window.parent.innerWidth <= 900 ? 'block' : 'none';
                });
                btn.addEventListener('click', () => {
                    const el = pd.documentElement;
                    const fs = pd.fullscreenElement || pd.webkitFullscreenElement;
                    if (!fs) {
                        (el.requestFullscreen || el.webkitRequestFullscreen || (() => {})).call(el);
                    } else {
                        (pd.exitFullscreen || pd.webkitExitFullscreen || (() => {})).call(pd);
                    }
                });
                pd.body.appendChild(btn);
            }

        } catch (e) { console.warn('Mobile fix error:', e); }
    })();
    </script>
    """,
    height=0, width=0
...
<style>
    ...
    * {{
    ...
    /* ========================================================
       PC 레이아웃 고정 – 650px 미만도 PC와 동일하게 렌더링
       ======================================================== */
    ...
    /* div 접두사로 Streamlit과 동일 명시도(specificity) 확보 → !important 충돌 시 우선 */
    ...
    /* Streamlit 640px 이하 세로 쌓기 미디어쿼리 완전 무력화 */
